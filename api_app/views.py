...
class EventList(APIView):
    def get(self, request):
        ...
        if queryset.exists():
            ...
            return Response(
                Event.objects.filter(country=request.GET.get("country")).values()
            )
...
class FlightList(APIView):
    def get_nearby_airport(self, event):
        ...
        url = f"https://airlabs.co/api/v9/nearby?lat={lat}&lng={lon}&distance=200&api_key=7fe8c5aa-d035-4298-9a12-443dbbd3b65c"
    ...
    def get(self, request):
        ...
        if nearest_ariport is None:
            return Response({"status": "error", "message": "No nearby airport found"})
        ...
        if queryset.exists():
            ...
            event = Event.objects.get(id=id)
            url_going = f'https://airlabs.co/api/v9/schedules?dep_icao={airport_code}&arr_icao={nearest_ariport["icao_code"]}&api_key=7fe8c5aa-d035-4298-9a12-443dbbd3b65c'
            url_back = f'https://airlabs.co/api/v9/schedules?dep_icao={nearest_ariport["icao_code"]}&arr_icao={airport_code}&api_key=7fe8c5aa-d035-4298-9a12-443dbbd3b65c'
            ...
            if response_going.status_code == 200 and response_back.status_code == 200:
                ...
                return Response(
                    Flight.objects.filter(event=id, airport_code=airport_code).values()
                )

            else:
                return Response(
                    {"message": "Failed to retrieve flights from AirLabs API"},
                    status=response_going.status_code,
                )